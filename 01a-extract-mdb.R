...
local({
  ...
  prev_archive <- file.path(dataPath, basename(extracted_mdb_tables_zip))
  ...
  withr::with_dir(outputPath, {
    archive::archive_write_files(
      basename(extracted_mdb_tables_zip),
      fs::path_rel(csv_files, outputPath)
    )
  })

  ## upload to Google Drive
  drive_id <- as_id("16OK48u6g5-JdWvEFhIJ0vMxvcK-k6z5l")
  drive_update(file = drive_id, media = extracted_mdb_tables_zip)
})
