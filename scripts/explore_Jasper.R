...
infile <- file.path("data", "Brett", "JasperData", "AreaInfested20132021.txt")
Jasper.AreaInfested <- read.table(infile, header = TRUE)
dev.new(height = 5, width = 5, noRStudioGD = TRUE)
plot(
  Jasper.AreaInfested$year,
  Jasper.AreaInfested$area,
  type = "l",
  xlab = "year",
  ylab = "area infested (ha)",
  main = "Jasper National Park"
)
points(Jasper.AreaInfested$year, Jasper.AreaInfested$area, pch = 19)

## plot interannual change in area infested, R
Jasper.R <- Jasper.AreaInfested$area[2:length(Jasper.AreaInfested$area)] /
  Jasper.AreaInfested$area[1:(length(Jasper.AreaInfested$area) - 1)]
dev.new(height = 5, width = 5, noRStudioGD = TRUE)
hist(Jasper.R)
