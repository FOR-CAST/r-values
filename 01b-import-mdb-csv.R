...
all_mpb_site_trees_cleaned_csv <- file.path(
  outputPath,
  "AB",
  "csv",
  "all_mpb_site_trees_cleaned.csv"
)

local({
  if (!file.exists(all_mpb_site_trees_cleaned_csv) || rerun_all) {
    ...
    abr <- dirname(dirname(csv_files)) |>
      ...
      purrr::map(function(d) {
        ...
        if (length(survey_site_csv) == 0) {
          survey_site_csv <- file.path(
            d_site,
            list.files(d_site, pattern = "_mpb_survey_info[.]csv")
          )
        }
...
abr <- read.csv(all_mpb_site_trees_cleaned_csv, header = TRUE)
